...
AMFA_inc <- function(Y,numTries = 2, eta = 0.5e-2 ,itmax = 500,
                    tol = 1e-05, conv_measure = "diff", 
                    nkmeans = 1, nrandom = 1, varimax = FALSE){
  
  #f_call = paste0("Call: AMFA_inc(Y, numTries = ", numTries ,", eta = ",eta,",itmax = ", itmax, ", tol = ", tol,"\n" ,", conv_measure = ", conv_measure, ", nkmeans = ", nkmeans, ", nrandom = ", nrandom, ", varimax = ",  varimax ,")")
  f_call = match.call()
  ...
  class(bestmodel) <- "MFA"
  bestmodel$diagnostics$data = Y
  bestmodel$diagnostics$call = f_call
