...
#'
#' @param Y An \emph{n} by \emph{p} data matrix, where \emph{n} is the number of
...
#'
amofa <- function(Y, itmax = 100, verbose  = FALSE, varimax = FALSE) {
  #
  #f_call = paste0("Call: amofa(Y, itmax = ", itmax ,", verbose = ",verbose,",varimax = ", varimax ,")")
  f_call = match.call()
  start.time <- Sys.time()
 ...
 #
  ...
  ffaOut <- ffa(Y, numFactors, maxIter)

  Lambda1 <- ffaOut$L
  Psi1 <- ffaOut$Ph
  logsTra <- ffaOut$LL
  Mu1 = matrix(colMeans(Y))
  ...
  DataLogLike = loglike3(Y, Lambda1, Psi1, Mu1, 1, numFactors, 1000)

  logsCv = DataLogLike
  #keeps track of validation loglike during the whole algorithm

  tLog = exp(loglike2(Y, Lambda1, Psi1, Mu1))
  ...
  while (cont_EM) {
    ...
    if (numMeans == 1) {
      ...
      if (direction < 0) {
        ...
        tempModel = cont_mfa_fj(Y, iMu, iLambda, iPsi, iPi, inumFactors, Inf, 10, verbose = verbose)
        ...
        tLog = exp(loglike4(Y, Lambda, Psi, Mu, Pi, numFactors))
    ...
    if (k > -1 & direction > 0) {
      ...
      if (prod(dim(localdata)) <= 1) {
        ...
        if (!is.infinite(minDL_l)) {
          ...
          if (reject == 0) {
            ...
            while (trials < maxTrial) {
              ...
              #
              dummyTC <- tryCatch( cont_mfa_fj(Y,
                          dumMu,
                          dumLambda,
                          dumPsi,
                          dumPi,
        ...
        if (k > -1 & direction > 0) {
          ...
          eval(parse(
            text = paste(
              'templocal2 = softresidual(Y,Lambda',
              ks,
              ',Psi',
              ks,
              ',tLogN[,k],Mu',
          ...
          while (trials < maxTrial) {
            ...
            dummyTC2 <- tryCatch(cont_mfa_fj(Y,
                                             Mu,
                                             dumLambda,
                                             dumPsi,
                                             Pi,
        ...
        if (!is.infinite(minDlenght)) {
         ...
         #
          ...
          tLog = exp(loglike4(Y, Lambda, Psi, Mu, t(Pi), numFactors))
...
for (i in smallLogIndices) {
  ...
  tLogN[i,] = tLogN[i,] / sum(tLogN[i,]);
    ...
    #
      ...
      loglikeVal <- loglike4(Y,bestmodel$Lambda,bestmodel$Psi,bestmodel$Mu,bestmodel$Pi,bestmodel$numFactors)
      sumLoglik <- logsumexp(loglikeVal,2)
      ll <- sum(sumLoglik)
      bestmodel$numFactors <- matrix(bestmodel$numFactors, nrow = 1)
      g <- dim(bestmodel$numFactors)[2]; p <- ncol(Y);
