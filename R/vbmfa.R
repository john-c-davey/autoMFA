...
#' @param numTries The maximum number of times the algorithm will attempt to split each component.
...
#'
vbmfa <- function(Y, qmax = NULL, numTries = 3, verbose = FALSE, varimax = FALSE){
  
  #f_call = paste0("Call: vbmfa(Y, qmax = ", qmax ,", numTries = ",numTries,", verbose = ", verbose, ", varimax = ", varimax, ")")
  f_call = match.call()
  k <- qmax
  if(is.null(k)){
    k = dim(Y)[2] 
...
if(verbose){
print(paste("Iteration: ", it, " Completion: ", ( tries/numTries+(pos-1)/(dim(candorder)[2]-1)/numTries )*100 , '%'))
} else {
