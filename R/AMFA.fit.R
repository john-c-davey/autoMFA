...
#'
#' @description An implementation of AMFA algorithm from \insertCite{WangWan-Lun2020Alom}{autoMFA}. The number of factors, \emph{q}, is estimated during the fitting process of each MFA model.
#' The best value of \emph{g} is chosen as the model with the minimum BIC of all candidate models in the range \code{gmin} <= \emph{g} <= \code{gmax}.
...
#' @param nrandom The number of randomly initialised models that will be used for each combination of \emph{g} and \emph{q}.
...
AMFA <- function(Y, gmin = 1, gmax =10, eta = 0.5e-2 ,itmax = 500, nkmeans = 5, nrandom = 5, tol = 1e-05,
  ...
  #f_call = paste0("Call: AMFA(Y, gmin = ", gmin ,", gmax = ",gmax,",eta = ", eta, ", itmax = ", itmax,"\n", ", nkmeans = ", nkmeans, ", nrandom = ", nrandom, ", tol = ", tol, ", conv_measure = ",  conv_measure ,", varimax = ", varimax, ")")
  f_call <- match.call()
  ...
  for (ii in 1:maxinit) {
    ...
    init_model_para <- try(init_est_para.mfa(Y, g, q,
                           ...
                           silent = TRUE)
    if (inherits(init_model_para, "try-error")) {
      when <- paste("At start", ii)
    ...
    estd_model <- est.mfa(init_para = init_model_para,
                          Y = Y, g = g, q = ceiling(p/2), itmax = itmax, tol = tol,
  ...
  if(gmax != gmin){
    ...
    if((model$g == gmin) && (gmin != 1)){warning("The final model uses the minimum number of components. Consider increasing gmax.")}
  }
  ...
  class(out) = "MFA"
  out$diagnostics$data = Y
  out$diagnostics$call = f_call
